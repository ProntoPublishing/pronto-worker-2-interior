...
class Test_ConverterFrontBackMatter(unittest.TestCase):
    ...
    def test_dedication_renders_centered_italic_with_cleardoublepage(self):
        """Per Doc 23 R-2.4 (Bucket B.4), dedication renders on its
        own recto page, italic, vertically centered. \\cleardoublepage
        on both sides ensures recto landing for the dedication AND
        for the next subtype.
        """
        block = {
            "id": "b_000001", "type": "paragraph",
        ...
        # R-2.4 — recto start + recto next via \cleardoublepage.
        self.assertEqual(out.count("\\cleardoublepage"), 2)

    # test_copyright_renders_flushleft_small removed in Bucket B.3.
    # Copyright is now system-generated by lib.title_page (R-2.3),
    # not produced by the converter from a role="front_matter"
    # subtype="copyright" block. The new copyright-page rendering is
    # tested in tests/test_doc23_v1.py:Test_R2_3_CopyrightPage.
