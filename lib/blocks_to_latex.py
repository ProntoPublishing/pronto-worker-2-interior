...
class BlocksToLatexConverter:
    ...
    def _render_front_matter(self, block: Dict[str, Any], ctx: Dict[str, Any]) -> str:
        """Front-matter heading. Subtype steers the layout:

        Doc 23 R-2.4 — dedication: own recto page, italic, vertically
          centered. Wrapped with \\cleardoublepage on both sides:
          leading to land on a recto, trailing so the next subtype
          also lands on a recto.

        Doc 23 R-2.6 — other front matter (foreword / preface /
          prologue / introduction / note_to_reader / epigraph /
          generic): each starts on a recto via leading
          \\cleardoublepage; rendered as unnumbered chapter-style
          headings (\\chapter*) so the text appears in the standard
          chapter heading style.

        Pre-Bucket-B "copyright" subtype branch removed — copyright
        is now system-generated by lib.title_page (R-2.3), not from
        a block. If a producer ever emits role="front_matter"
        subtype="copyright", it falls through to the generic
        rendering, which is harmless (renders as an unnumbered
        chapter-style heading rather than nothing).
        """
        subtype = (block.get("subtype") or "generic").lower()
        text_body = self._plain(block)
        ...
        # R-2.6 — foreword / preface / prologue / introduction /
        # note_to_reader / epigraph / generic. Recto start via
        # \cleardoublepage; unnumbered chapter heading.
        return (
            "\\cleardoublepage\n"
            f"\\chapter*{{{text_body}}}\n"
...
# Doc 23 §Front Matter — canonical subtype rendering order. Per Q1
# of Bucket B planning: "Re-order by subtype. Doc 23's posture is
# house-style-enforced; trusting the artifact's source order would
# let a misordered manuscript produce a non-Standard-Edition book."
#
# Order rationale:
#   dedication first (R-2.4, its own page).
#   foreword → preface → prologue: traditional sequence; foreword is
#     someone else's words, preface is the author's, prologue is
#     in-fiction setup.
#   introduction → note_to_reader: scholarly / didactic apparatus.
#   epigraph: per R-2.7 deferred, but if present render here so it
#     lands before the body (a standalone epigraph block is unusual;
#     more often handled inline by W1).
#   Anything else (including the catch-all "generic") sorts last via
#     the dict.get default; Python's sort is stable so within-rank
#     order is preserved.
_FRONT_MATTER_SUBTYPE_ORDER: Dict[str, int] = {
    "dedication":     0,
    "foreword":       10,
    "preface":        20,
    "prologue":       30,
...
_DEFAULT_FRONT_MATTER_RANK = 100


def _front_matter_sort_key(block: Dict[str, Any]) -> int:
    subtype = (block.get("subtype") or "generic").lower()
    return _FRONT_MATTER_SUBTYPE_ORDER.get(subtype, _DEFAULT_FRONT_MATTER_RANK)
...
def _partition_front_matter(
    ...
    The front-matter partition is RE-ORDERED by canonical subtype
    rank per Doc 23 (dedication first, then foreword → preface →
    prologue → introduction → note_to_reader → epigraph, then
    anything else). Within each rank, Python's stable sort preserves
    the source document order — so two prefaces (rare but possible)
    keep their relative order.

    Body partition preserves document order. Pure function; does
    not mutate input. Roles in `_CONSUMED_ROLES` (currently only
    ...
    for block in blocks:
        ...
        if role in _FRONT_MATTER_ROLES:
            ...
            body.append(block)
    front.sort(key=_front_matter_sort_key)
    return front, body
